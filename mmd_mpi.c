...
/*
float a[4096][4096], b[4096][4096], c[4096][4096];
*/
...
/* 檢查是否有輸入矩陣大小參數*/
      if (argc != 2)
      {
         printf("please input matrix size，eg. mm_mpi 256\n");
         MPI_Finalize();
         return 1;
      }
   }
...
/*
   printf("CPU %d is calculate between (%4d~%4d) in %d\n", mpi_rank, node_start, node_end, Matrix_Size);
   time_start=MPI_Wtime();
*/
/* 傳送 a 矩陣資料 */
   if (mpi_rank == MASTER)
